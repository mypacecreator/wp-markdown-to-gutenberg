...
```
{{reuse:42}}
{{reuse:my-reusable-block}}
```

WordPressの再利用ブロックを呼び出す。投稿IDまたはスラッグで指定できる。

| 記法例 | 動作 |
|--------|------|
| `{{reuse:42}}` | 投稿ID 42 の再利用ブロックを挿入 |
| `{{reuse:my-block}}` | スラッグ `my-block` の再利用ブロックを検索して挿入 |

- スラッグ指定時は、エディター起動時にプリフェッチした `wp_block` のうち先頭 100 件から ID を解決する
- スラッグに一致する再利用ブロックが存在しない、またはプリフェッチした 100 件に含まれていない場合はサイレント無視（ブロックは挿入されない）
